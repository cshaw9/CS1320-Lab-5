...
const madlibs = [
   ...
   },
   {
       ...
       author: 'unknown',
       content: 'Running through Cyberspace, the team encounters a [adjective] [noun].  Unsure of ' +
   ...
   },
   {
       ...
       author: 'unknown',
       content: 'Hacker is at it again and he tries to convince that team that any [number] ' +
   ...
   },
   {
       title: 'A Tall Tale',
       author: 'unknown',
       content: '[number] years ago, Motherboard was brought to life.  Motherboard could [verb] ' +
   ...
   },
   {
       title: 'Black Hole',
       author: 'unknown',
       content: 'There is a Black Hole that is [verb] by a sentry named [noun] who charges ' +
...
{ 
   ...
   */
   ...
   req.session.prevIndex = req.session.currIndex;
   let nextIndex = Math.floor(Math.random() * madlibs.length);
   while (nextIndex == req.session.prevIndex) {    // prevent repeats
      nextIndex = Math.floor(Math.random() * madlibs.length);
   }
   req.session.currIndex = nextIndex;
   req.session.save();
...
function displayResult(req,res)
{
   ...
   */
   const currIndex = req.session.currIndex;  // get what madlib we used

   const words = [];
   for (key in req.body) {
      words.push(req.body[key]);
   }

   res.render('showmadlib', { body: replaceItems(madlibs[currIndex], words) });
}
